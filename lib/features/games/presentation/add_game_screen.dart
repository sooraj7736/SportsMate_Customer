    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          _buildCardFrame("Match Context", [
            ...
            if (_isOtherLocation) ...[
              ...
              TextFormField(
                ...
                decoration: const InputDecoration(labelText: 'Custom Address / Map Details', border: InputBorder.none, prefixIcon: Icon(Icons.map)),
              ),
              ...
              ElevatedButton.icon(
                ...
                style: ElevatedButton.styleFrom(
                  backgroundColor: _latitude != null ? const Color(0xFF1DB954) : Colors.blue,
          ...
          ElevatedButton(
            onPressed: (_isOtherLocation && _customAddressController.text.isEmpty) 
                ? null 
                : () async {
                    ...
                    if (_isOtherLocation) {
                      ...
                      latToUse = turf['lat']?.toDouble() ?? turf['latitude']?.toDouble();
                      lngToUse = turf['lng']?.toDouble() ?? turf['longitude']?.toDouble();
                    }
                    ...
                    // Send notifications to users within 10km
                    if (latToUse != null && lngToUse != null) {
                      final activeAddressesSnapshot = await FirebaseFirestore.instance
                      ...
                      for (final doc in activeAddressesSnapshot.docs) {
                        ...
                        final addrLat = (addrData['lat'] as num?)?.toDouble() ?? 0.0;
                        final addrLng = (addrData['lng'] as num?)?.toDouble() ?? 0.0;
                        
                        final distance = _calculateDistance(latToUse, lngToUse, addrLat, addrLng);
                        if (distance <= 10.0) {
